      ...
      wx.chooseImage({
        ...
        success: async res => {
          ...
          const uploadTask = wx.cloud.uploadFile({
            cloudPath: `聊天室/${this.data.groupId}/${this.data.userInfo.nickName}/${Math.random()}_${Date.now()}.${res.tempFilePaths[0].match(/\.(\w+)$/)[1]}`,
            /*`${this.data.openId}/${Math.random()}_${Date.now()}.${res.tempFilePaths[0].match(/\.(\w+)$/)[1]}`,*/
